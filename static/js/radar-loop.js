...
(function(){
  ...
  var FLASH_HOURS=48, FLASH_PLAN_PCT=40, FLASH_PLAN_EUR=19.99;
  ...
  function flashBannerHTML(){
    ...
    var now=Math.round(FLASH_PLAN_EUR*(1-FLASH_PLAN_PCT/100)*100)/100;        // 19.99 → 11.99
    var nowTxt=(now%1?String(now.toFixed(2)).replace(".",","):String(now));
    var wasTxt=(FLASH_PLAN_EUR%1?String(FLASH_PLAN_EUR.toFixed(2)).replace(".",","):String(FLASH_PLAN_EUR));
    return '<div class="flash-offer" data-act="flash-cta" role="button" tabindex="0" aria-label="Oferta: Creator a −40% el primer mes">'+
      ...
      '<div class="flash-txt"><b>'+L("Sigue con Creator · −40% tu primer mes","Stay on Creator · −40% your first month")+'</b>'+
        '<span>'+L("Lo que ChatGPT no hace: te digo qué explota y te lo robo en tu voz","What ChatGPT won't: I tell you what's blowing up and steal it in your voice")+' · <s>€'+wasTxt+'</s> → <b>€'+nowTxt+'</b> · WELCOME ✓</span></div>'+
  ...
  function loadAll(){
    ...
    Promise.all([
      ...
      // (Leo #266) El «tour visto» (onboarding_completed) vive en localStorage, NO en la
      // cuenta → un reset de cuenta (onb_v2_done=false) no lo revertía y el tour no se
      // re-mostraba. Si el servidor dice que el onboarding NO está hecho, limpiamos el flag
      // local para que el house tour vuelva a salir.
